...
public class Radar
{
    ...
    {
        if (!ZoneData.IsInOccultCrescent()) return;

        if (!module.config.ShouldDrawLineToCarrots) return;

        if (Svc.ClientState.LocalPlayer == null || Svc.Condition[ConditionFlag.InCombat]) return;
