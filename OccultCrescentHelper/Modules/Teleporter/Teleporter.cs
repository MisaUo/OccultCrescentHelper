...
public class Teleporter
{
    ...
    {
        if (!module.TryGetIPCProvider<VNavmesh>(out var vnav) || vnav == null || !vnav.IsReady()) return;

        if (aethernet == null) aethernet = GetClosestAethernet(destination);
    ...
    private void PathfindingButton(Vector3 destination, string name, string id, EventData ev)
    {
        ...
        if (ImGui.IsItemHovered()) ImGui.SetTooltip($"Pathfind to {name}");


        if (!module.TryGetIPCProvider<Lifestream>(out var lifestream) || lifestream == null ||
            !lifestream.IsReady()) return;
    ...
    {
        if (module.GetModule<AutomatorModule>().enabled) return;

        if (!module.config.ReturnAfterFate) return;
    ...
    public void OnCriticalEncounterEnd()
    {
        ...
        if (!module.config.ReturnAfterCritcalEncounter) return;
    ...
    public void Return()
    {
        if (ZoneData.IsInForkedTower()) return;
