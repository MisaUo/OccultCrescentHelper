...
public class Radar
{
    ...
    {
        if (!ZoneData.IsInOccultCrescent() || Svc.Condition[ConditionFlag.InCombat]) return;

        var config = module.config;
        if (!config.ShouldDrawLineToBronzeChests && !config.ShouldDrawLineToSilverChests) return;
